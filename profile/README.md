...
1. Clone **[deploy](https://github.com/git-lfs-hub/deploy)**, the monorepo that ties everything together.
2. Edit `vars.input.json` with your Cloudflare account, R2 bucket name, GitHub org, and OAuth app details.
3. Run `bun run config` — the **[config](https://github.com/git-lfs-hub/config)** renderer merges your inputs with defaults, validates against a JSON Schema, and writes out `wrangler.jsonc` and `github-app.md` (instructions for creating the GitHub App).
4. Run `bun run deploy` — **[docs](https://github.com/git-lfs-hub/docs)** is built into the Worker's static assets, **[server](https://github.com/git-lfs-hub/server)** is bundled and shipped to Cloudflare.
5. Point your team's `.lfsconfig` at your new endpoint, sign in once via GitHub, and `git lfs push` works.

## The repositories

Each repo is independently useful, but **[deploy](https://github.com/git-lfs-hub/deploy)** is the entry point — it consumes the other four as workspaces or submodules.

| Repo | Description |
|:-----|:------------|
| **[deploy](https://github.com/git-lfs-hub/deploy)**<br/>[![deploy][deploy-badge]][deploy-href] | The Turbo monorepo and your starting point. Holds `vars.input.json`, the deploy scripts, the GitHub Actions workflows (`pr.yml`, `main.yml`), and submodules for `server/`, `docs/`, and `e2e/`. Clone this first. |
| **[server](https://github.com/git-lfs-hub/server)**<br/>[![server][server-badge]][server-href] | The Worker source: a Hono app implementing the LFS batch API, OAuth callback, R2 presigning, and Durable Object lock store. Use this repo directly if you want to embed the Worker into a different deploy story and manage Wrangler config yourself. |
| **[docs](https://github.com/git-lfs-hub/docs)**<br/>[![docs][docs-badge]][docs-href] | The end-user documentation site, built with DocMD. Covers installing Git LFS, configuring remotes, and using your LFS Hub instance. Built into `server/public/` as part of the deploy pipeline; edit it in isolation if you just want to tweak guides. |
| **[config](https://github.com/git-lfs-hub/config)**<br/>[![config][config-badge]][config-href] | The vars renderer, invoked as `bunx github:git-lfs-hub/config` (or `bun run config` from a deploy checkout). Merges `vars.input.json` with package defaults, Ajv-validates against the schema, and renders the Handlebars templates `wrangler.template.jsonc` and `github-app.template.md` into their final forms. Idempotent on re-run. |
| **[e2e](https://github.com/git-lfs-hub/e2e)**<br/>[![e2e][e2e-badge]][e2e-href] | The end-to-end test harness that proves each release works before it ships. A reusable GitHub Actions workflow (`staging.yml`) plus a vitest suite (`test-docs`, `test-git-lfs`) deploys a throwaway `-staging` Worker and exercises a real `git lfs push` against it on every change to the upstream project. |
...
[deploy-href]: https://github.com/git-lfs-hub/deploy/actions/workflows/main.yml?query=branch%3Amain

[server-badge]: https://badgen.net/github/checks/git-lfs-hub/server/main?icon=vitest&label=
[server-href]: https://github.com/git-lfs-hub/server/actions/workflows/main.yml?query=branch%3Amain

[docs-badge]: https://badgen.net/github/checks/git-lfs-hub/docs/main?icon=markdown&label=
[docs-href]: https://github.com/git-lfs-hub/docs/actions/workflows/main.yml?query=branch%3Amain

[config-badge]: https://badgen.net/github/checks/git-lfs-hub/config/main?icon=vitest&label=
[config-href]: https://github.com/git-lfs-hub/config/actions/workflows/main.yml?query=branch%3Amain

[e2e-badge]: https://badgen.net/github/checks/git-lfs-hub/e2e/main?icon=bun&label=
[e2e-href]: https://github.com/git-lfs-hub/e2e/actions/workflows/main.yml?query=branch%3Amain
